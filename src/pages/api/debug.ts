...
export const GET: APIRoute = async ({ url }) => {
  ...
  try {
    ...
    if (test === "guides") {
      ...
      return json({ ok: true, count: guides.length, first: guides[0]?.title, blocks: guides[0]?.content.length });
    }
    if (test === "settings") {
    ...
    return new Response(JSON.stringify({ error: String(e), stack: (e as Error).stack?.split("\n").slice(0, 5) }), {
      status: 500,
