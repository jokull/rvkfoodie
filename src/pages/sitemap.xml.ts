...
export const GET: APIRoute = async () => {
  ...
  for (const guide of guides) {
    urls.push({ loc: `/guides/${guide.slug}`, priority: "0.9", changefreq: "weekly" });
    for (const block of guide.content) {
      if (block.blockType === "section") {
        for (const venue of (block as SectionBlock).venues) {
          urls.push({ loc: `/places/${venue.id}`, priority: "0.7", changefreq: "monthly" });
        }
      }
    }
  }
  ...
  for (const post of editorials) {
    urls.push({ loc: `/blog/${post.slug}`, priority: "0.8", changefreq: "monthly", lastmod: post.date?.split("T")[0] });
  }
